...
const client = new WAConnection()
   ...
   console.log(color('[','white'),color('∆','red'),color(']','white'),color('qr already scan.subscribe','white'),color('TELE','red'),color('GRAM','white'),color('MR D1CK','yellow'))
})
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["13132900007@s.whatsapp.net"] 
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				case 'bc': 
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `*「 BROADCAST MRDBOTV3 」*\n\n${body.slice(4)}`)
						}
						reply('𝙨𝙪𝙘𝙘𝙚𝙨𝙨 𝙗𝙧𝙤𝙖𝙙𝙘𝙖𝙨𝙩 ')
					}
					break
